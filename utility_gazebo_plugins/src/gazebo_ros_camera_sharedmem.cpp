...
#include <gazebo_ros_camera_sharedmem.hpp>
...
class GazeboRosCameraSharedMemPrivate
{
public:
  ...
  std::vector<rclcpp::Publisher<sensor_msgs::msg::CompressedImage>::SharedPtr> image_jpg_pub_;
...
void GazeboRosCameraSharedMem::OpenSharedMemory(void)
{   
    const char * shm_name  = impl_->camera_name_.c_str();
    impl_->m_fd = shm_open(shm_name, O_RDWR, 0);
    if (impl_->m_fd < 0) {
        // failed to open existing file, try to create a new one
        impl_->m_fd = shm_open(shm_name, O_RDWR | O_CREAT, 0666);
        if (impl_->m_fd < 0 || ftruncate(impl_->m_fd, SHARED_MEMORY_SIZE) != 0) {
            return;
        }
    }
    impl_->m_map = static_cast<char*>(
        mmap(NULL, SHARED_MEMORY_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, impl_->m_fd, 0));
}

void GazeboRosCameraSharedMem::CloseSharedMemory(void)
{
    if (impl_->m_map != NULL) {
      const char * shm_name  = impl_->camera_name_.c_str();
      munmap(impl_->m_map, SHARED_MEMORY_SIZE);
      impl_->m_fd = shm_unlink(shm_name);
    }

}

GazeboRosCameraSharedMem::GazeboRosCameraSharedMem()
: impl_(std::make_unique<GazeboRosCameraSharedMemPrivate>())
{
}
...
void GazeboRosCameraSharedMem::Load(gazebo::sensors::SensorPtr _sensor, sdf::ElementPtr _sdf)
{
  /*
  ...
  if (std::dynamic_pointer_cast<gazebo::sensors::MultiCameraSensor>(_sensor)) {
    impl_->sensor_type_ = GazeboRosCameraSharedMemPrivate::MULTICAMERA;
    gazebo_plugins::MultiCameraPlugin::Load(_sensor, _sdf);
    impl_->num_cameras_ = gazebo_plugins::MultiCameraPlugin::parent_sensor_->CameraCount();
  } else if (std::dynamic_pointer_cast<gazebo::sensors::DepthCameraSensor>(_sensor)) {
    impl_->sensor_type_ = GazeboRosCameraSharedMemPrivate::DEPTH;
    gazebo::DepthCameraPlugin::Load(_sensor, _sdf);
  } else if (std::dynamic_pointer_cast<gazebo::sensors::CameraSensor>(_sensor)) {
    impl_->sensor_type_ = GazeboRosCameraSharedMemPrivate::CAMERA;
  ...
  if (impl_->sensor_type_ != GazeboRosCameraSharedMemPrivate::MULTICAMERA) {
    // Image publisher
    ...
    const std::string camera_jpg_topic = impl_->camera_name_ + "/image_jpg";

    impl_->image_jpg_pub_.push_back(
        impl_->ros_node_->create_publisher<sensor_msgs::msg::CompressedImage>(
          camera_jpg_topic, qos.get_publisher_qos(
            camera_jpg_topic, rclcpp::SensorDataQoS().reliable())));
    ...
    for (uint64_t i = 0; i < impl_->num_cameras_; ++i) {
      const std::string camera_topic =
        impl_->camera_name_ + "/" + gazebo_plugins::MultiCameraPlugin::camera_[i]->Name() + "/image_raw";
      // Image publisher
      impl_->image_pub_.push_back(
        image_transport::create_publisher(
          impl_->ros_node_.get(), camera_topic, qos.get_publisher_qos(
      ...
      //   gazebo_plugins::MultiCameraPlugin::camera_[i]->Name().c_str(),
      //   impl_->image_pub_.back().getTopic());

      const std::string camera_info_topic =
        impl_->camera_name_ + "/" + gazebo_plugins::MultiCameraPlugin::camera_[i]->Name() + "/camera_info";
      // Camera info publisher
      impl_->camera_info_pub_.push_back(
        impl_->ros_node_->create_publisher<sensor_msgs::msg::CameraInfo>(
          camera_info_topic, qos.get_publisher_qos(
  ...
  if (impl_->sensor_type_ == GazeboRosCameraSharedMemPrivate::DEPTH) {
  ...
  if (_sdf->Get<bool>("triggered", false).first) {
    ...
    impl_->trigger_sub_ = impl_->ros_node_->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&GazeboRosCameraSharedMem::OnTrigger, this, std::placeholders::_1));
    ...
    impl_->pre_render_connection_ = gazebo::event::Events::ConnectPreRender(
      std::bind(&GazeboRosCameraSharedMem::PreRender, this));
  }

  // Buffer size
  std::vector<std::string> image_format;
  if (impl_->sensor_type_ == GazeboRosCameraSharedMemPrivate::CAMERA) {
    image_format.push_back(CameraPlugin::format);
  } else if (impl_->sensor_type_ == GazeboRosCameraSharedMemPrivate::DEPTH) {
    image_format.push_back(DepthCameraPlugin::format);
  } else {
    image_format = gazebo_plugins::MultiCameraPlugin::format_;
  }
  ...
  if (impl_->sensor_type_ == GazeboRosCameraSharedMemPrivate::CAMERA) {
    ...
    width = gazebo_plugins::MultiCameraPlugin::width_;
    height = gazebo_plugins::MultiCameraPlugin::height_;
    impl_->camera_ = gazebo_plugins::MultiCameraPlugin::camera_;
  }
  ...
  if (impl_->sensor_type_ == GazeboRosCameraSharedMemPrivate::CAMERA) {
    ...
    impl_->ros_node_->declare_parameter(
      "update_rate", gazebo_plugins::MultiCameraPlugin::parent_sensor_->UpdateRate());
  }
          ...
          if (nullptr != impl_->trigger_sub_) {
            ...
            if (rclcpp::ParameterType::PARAMETER_DOUBLE == parameter_type) {
              ...
              if (impl_->sensor_type_ == GazeboRosCameraSharedMemPrivate::CAMERA) {
                ...
                gazebo_plugins::MultiCameraPlugin::parent_sensor_->SetUpdateRate(rate);
              }
...
void GazeboRosCameraSharedMem::NewFrame(
  ...
  const int _camera_num)
{
  ...
  if (impl_->sensor_type_ == GazeboRosCameraSharedMemPrivate::CAMERA) {
    ...
    sensor_update_time = gazebo_plugins::MultiCameraPlugin::parent_sensor_->LastMeasurementTime();
  }
  ...
  size_t st0 = (impl_->img_step_[_camera_num] * _width * _height);
  /*
  RCLCPP_DEBUG(
      impl_->ros_node_->get_logger(),
      "copying new frame ***********  %x, %d, %d, %d, %d", impl_->m_map , st0, _width, _height, impl_->img_step_[_camera_num]);
  */
  std::memcpy(impl_->m_map, reinterpret_cast<const void *>(_image), st0);
  
    
  cv_bridge::CvImagePtr cv_ptr;
  cv_ptr = cv_bridge::toCvCopy(impl_->image_msg_, sensor_msgs::image_encodings::BGR8);
  sensor_msgs::msg::CompressedImage::SharedPtr compressedImage = cv_ptr->toCompressedImageMsg();
  impl_->image_jpg_pub_[_camera_num]->publish(*compressedImage);
...
////////////////////////////////////////////////////////////////////////////////
void GazeboRosCameraSharedMem::OnNewFrame(
  const unsigned char * _image,
  unsigned int _width,
  unsigned int _height,
  unsigned int /*_depth*/,
...
{
#ifdef IGN_PROFILER_ENABLE
  IGN_PROFILE("GazeboRosCameraSharedMem::OnNewFrame");
...
////////////////////////////////////////////////////////////////////////////////
void GazeboRosCameraSharedMem::OnNewImageFrame(
  const unsigned char * _image,
  unsigned int _width,
  unsigned int _height,
  unsigned int /*_depth*/,
...
////////////////////////////////////////////////////////////////////////////////
void GazeboRosCameraSharedMem::OnNewDepthFrame(
  const float * _image,
  unsigned int _width,
  unsigned int _height,
  unsigned int /*_depth*/,
...
{
#ifdef IGN_PROFILER_ENABLE
  IGN_PROFILE("GazeboRosCameraSharedMem::OnNewDepthFrame");
...
////////////////////////////////////////////////////////////////////////////////
void GazeboRosCameraSharedMem::OnNewMultiFrame(
  const unsigned char * _image,
  unsigned int _width,
  unsigned int _height,
  unsigned int /*_depth*/,
...
void GazeboRosCameraSharedMem::SetCameraEnabled(const bool _enabled)
{
  if (impl_->sensor_type_ == GazeboRosCameraSharedMemPrivate::CAMERA) {
    ...
    gazebo_plugins::MultiCameraPlugin::parent_sensor_->SetActive(_enabled);
    gazebo_plugins::MultiCameraPlugin::parent_sensor_->SetUpdateRate(_enabled ? 0.0 : ignition::math::MIN_D);
  }
}

void GazeboRosCameraSharedMem::PreRender()
{
  std::lock_guard<std::mutex> lock(impl_->trigger_mutex_);
...
void GazeboRosCameraSharedMem::OnTrigger(const std_msgs::msg::Empty::SharedPtr)
{
  std::lock_guard<std::mutex> lock(impl_->trigger_mutex_);
  impl_->triggered++;
}
...
{
  return (gazebo::CameraPlugin *)(new GazeboRosCameraSharedMem());
}
